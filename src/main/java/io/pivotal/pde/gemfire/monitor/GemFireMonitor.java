	...
	public void run() {
		...
		while (running.get()) {
			...
			try {
				...
				ObjectName dsOname = new ObjectName(
						"GemFire:service=System,type=Distributed");
				DistributedSystemMXBean distributedSystemBean = JMX.newMXBeanProxy(
						mbsc, dsOname, DistributedSystemMXBean.class);
				...
				logMetric(dsOname.toString(), "TotalRegionEntryCount", distributedSystemBean.getTotalRegionEntryCount());
				...
				for(ObjectName memberOName: memberONames){
					...
					logMetric(memberOName.toString(), "NumThreads", memberMBean.getNumThreads());
				...
				running.set(false);  // will keep the shutdown method from waiting
				break; //BREAK
			} finally {
				try { 
					jmxc.close();
